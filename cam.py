...
class StereoConfig:
    ...
    def __init__(self):
        ...
        self.blockSize = 13
        self.P1 = 8 * self.blockSize * self.blockSize
        self.P2 = 32 * self.blockSize * self.blockSize
...
# 新增函数：创建并可视化深度图
def create_depth_map(points_3d, config):
    """
    从3D点云创建深度图
    """
    # 提取Z坐标，即深度值
    ...
    # 过滤异常值
    min_depth = config.min_distance_mm  # 最小深度值 (mm)
    max_depth = config.max_distance_mm  # 最大深度值 (mm)

    # 将范围外的值设为0
    depth_map[(depth_map < min_depth) | (depth_map > max_depth)] = 0

    # 归一化到0-255以便显示
    valid_mask = depth_map > 0
    if np.any(valid_mask):
        min_valid = np.min(depth_map[valid_mask])
        max_valid = np.max(depth_map[valid_mask])

        # 避免除以零
        if max_valid > min_valid:
            # 归一化
            normalized = np.zeros_like(depth_map)
            normalized[valid_mask] = 255 * (1 - (depth_map[valid_mask] - min_valid) / (max_valid - min_valid))
            depth_normalized = normalized.astype(np.uint8)
        else:
    ...
    bar_width = 20
    for i in range(h):
        color_value = int(255 * (h - i - 1) / h)
        bar_color = cv2.applyColorMap(np.array([[color_value]], dtype=np.uint8), cv2.COLORMAP_JET)[0, 0]
        depth_color[i, w - bar_width:w] = bar_color

    # 添加最小值和最大值标签
    if np.any(valid_mask):
        cv2_put_chinese_text(depth_color, f"{min_valid / 1000:.2f}m", (w - bar_width - 70, h - 10),
                             cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
        cv2_put_chinese_text(depth_color, f"{max_valid / 1000:.2f}m", (w - bar_width - 70, 20),
                             cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)

    return depth_normalized, depth_color
...
class StereoVisionApp:
    ...
    def __init__(self):
        ...
        # 为深度图窗口也设置鼠标回调
        cv2.setMouseCallback(self.config.window_names["depth"], self.mouse_callback)
    ...
    def run(self):
        ...
        try:
            while self.running:
                ...
                # 创建深度图 (新增)
                _, depth_color = create_depth_map(points_3d, self.config)
                ...
                if self.config.measure_points:
                    ...
                    # 在深度图上也绘制相同的测量点
                    depth_with_measures = draw_measure_points(depth_color, self.config.measure_points, points_3d)
                    cv2.imshow(self.config.window_names["depth"], depth_with_measures)
                else:
